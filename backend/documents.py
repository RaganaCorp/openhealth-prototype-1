...
def upsert_summary_overrides_section(patient_folder: Path, overrides: Optional[dict]) -> None:
    """Write or replace a dedicated summary-corrections block in patient.md."""
    f = patient_folder / "patient.md"
    current = f.read_text(encoding="utf-8") if f.exists() else ""

    data = overrides or {}
    sections = [
        ("Active Conditions", str(data.get("active_conditions", "")).strip()),
        ("Current Medications", str(data.get("current_medications", "")).strip()),
        ("Recent Procedures", str(data.get("recent_procedures", "")).strip()),
        ("Key Concerns", str(data.get("key_concerns", "")).strip()),
    ]

    lines = [
        "<!-- SUMMARY_OVERRIDES_START -->",
        "## Caregiver Summary Corrections",
        "Use these corrections as preferred wording and focus when generating summaries.",
        "",
    ]
    has_any = False
    ...
    if not has_any:
        lines.append("No section-level corrections provided yet.")
        lines.append("")

    lines.append("<!-- SUMMARY_OVERRIDES_END -->")
    block = "\n".join(lines).strip()

    pattern = r"<!-- SUMMARY_OVERRIDES_START -->[\s\S]*?<!-- SUMMARY_OVERRIDES_END -->"
